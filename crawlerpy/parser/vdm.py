...
class VdmParser(Parser):
    ...
    def parse(self, text):
        try:
            ...
            if "VDM : Aujourd" in soup.find("title").getText():#only one article
                ...
                section.add_content(Data("string", text.getText().strip()))
                a = Article(l)
                a.add_section(section)
                reponse.append(a)
            else:#in multi article
                ...
                for article in articles:
                    ...
                    if 'http://www.public.fr' in content.get("href") or "http://people.premiere.fr" in content.get(
                            "href"):
                        continue
